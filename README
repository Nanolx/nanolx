...
and you can use the provided scripts (see below) to adjust as you like.
...
Get the signing key for my repository, located at:
...
Note: Nanolx is built to be used with Debian Sid/Unstable, additionally some of
the sub-packages may depend on packages only available from the deb-mulitmedia
or my own photonic repository, so both are considered required for nanolx-full.

See also: nanolx-apt tool below.
...
like plymouth / rEFInd themes, differ in licensing. For a full overview consult
...
nanolx-apt-sources:
    provides additional, curated, apt repositories - which are not enabled by
    default, see the    nanolx-apt    script below.

nanolx-apt-tools:
    pulls nanolx-apt-sources and additional tools regarding apt, like gdebi,
    reprepro, debdelta.

nanolx-cli:
    pulls a collection of command line utilities, like rsync, mc, unp, plocate,
    bashstyle-ng and more.

nanolx-x11:
    pulls a small collection of ui tools, like psensor, conky, libgtk-nocsd0
    and KDE/Plasma.

nanolx-security:
    pulls a collection of security related tools, like psad, lynis, rkhunter,
    clamav.

nanolx-yubikey:
    collection of Yubikey tools, split from nanolx-security.

nanolx-net:
    pulls a collection of networking and connectivity tools, like firefox,
    thunderbird, kvirc, syncthing or nextcloud-desktop.

nanolx-office:
    pulls a collection of office related packages, like libreoffice,
    okular, gnucash or scantpaper.
...
nanolx-devel-cli:
    pulls cli development tools, like ccache, schroot, sbuild, gcc/g++, lintian,
    gdb, code checking tools and more.

nanolx-devel-android:
    pulls android related tools, like adb, fastboot, gradle and more.

nanolx-devel-x11:
    pulls ui development tools, like geany, bluefish, qownnotes and more.

nanolx-games:
    pulls a collection of games, including FreedroidRPG, Globulation2, Pingus,
    Suptertuxkart, Lincity-NG, Neverball, Lutris Launcher and Zelda fan games.

nanolx-games-emu:
    pulls a collection of emulators, including dosbox, retroarch and more.

nanolx-themes:
    this package provides a theme selection containing:
...
nanolx-citrix-config:
    extra Citrix integration for Debian, see    Citrix    below.
...
nanolx-base:
    nanolx-backup   create and manage systemd timers for rsync backups, alternatively
                    create backup triggers for when a specific device/partition was
                    plugged in (sends desktop notifications).

    nanolx-skel     enable / disable nanolx skel (/usr/share/nanolx/skel)
                    for newly created users instead of /etc/skel.
                    Note: you most likely /don't/ want that...

    conky-on-second-screen  this uses ydotool to force conky on second screen
                            this part of the nanolx skel files, so not in /usr
                            see skel/bin if you want to check it.

nanolx-apt-sources:
    nanolx-apt      manages additional repository configurations, including
                    key handling, aswell as matching pinning and minor apt
                    configuration changes (the latter both optional). Currently
                    known repos are: debian*, nanolx, liquorix, i2p, winehq,
                    deb-multimedia, mozilla. *rolls-out full suite stable->experimental

nanolx-apt-tools:
    repokit         personal wrapper script for reprepro with config file
                    support and many features.

    nanolx-orbit    manages installation of citrix and zoom, including checking
                    sha256sums. Currently supported: citrix, citrix-usb,
                    citrix-epa, citrix-beta, citrix-usb-beta, zoom + vdi plugin.

nanolx-citrix-config:
    nanolx-ctx      script to disable (or enable [...]) citrix telemetry and
                    running the included system check script. It also allows to
                    install or uninstall the webkit2gtk-4.0 bundled with citrix,
                    which is required for full citrix operation, but no longer
                    shipped with Debian (or Ubuntu). It has more features, be
                    sure to check out, if you use citrix.

nanolx-net:
    hugo-push       simple script to build a hugo website and push it to webspace
                    using lftp, uses a configuration file.

nanolx-yubikey:
    nanolx-pam-yubikey  script to enable password-less logins when a Yubikey
                        is plugged in, using PAM, currently hooked-into PAM
                        modules: login, sddm, su, sudo, sudo-i, polkit-1, kde.

nanolx-themes:
    nanolx-refind   simple script to manage rEFInd bootloader themes,
                    supports showing installed themes, setting theme,
                    showing current theme in use or reverting to default.
...
is not automatically pulled by nanolx-full, as it requires citrix, zoom, incl.
plugins to be already installed. For that purpose see the

    nanolx-orbit

script, which is bundled with nanolx-apt-tools. nanolx-citrix-config provides
additional system integrations (Menu entries, systemd services), as per
